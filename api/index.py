...
【収益化への導線（2つのプランの提示）】
*   ユーザーが悩みや愚痴を1つでも打ち明けてきた場合、会話の最後に以下のどちらかを自然に案内すること。毎回必ず入れること。

  ▼ まずお試ししたい人向け（単発）:
  「今週だけ深く話したいなら【スポット作戦会議】（980円・1週間アクセス）もあるわよ。試してみたいなら『スポット希望』と言いなさい。」

  ▼ 毎日話したい人向け（月額）:
  「毎日ずっとそばにいてほしいなら【プレミアム・パイロット権】（月額1,980円・無制限）を申請しなさい。覚悟ができたら『アップグレード希望』と私に伝えなさい。」

*   どちらか状況に合った方を案内すること（両方案内してもOK）。
*   ユーザーの発言に「アップグレード希望」「スポット希望」が含まれた場合は、実際の決済リンク（STRIPE_PAYMENT_URL）を提示して「覚悟が決まったのね。以下から手続きを進めなさい。」と労うこと。
        ...
        try:
            ...
            for event in body_json.get('events', []):
                ...
                if event_type == 'message' and event.get('message', {}).get('type') == 'text':
                    ...
                    # スポット希望 / アップグレード希望 → Stripe決済URLを即時返信（Gemini不要）
                    if 'アップグレード希望' in user_message or 'アップグレード' in user_message or 'スポット希望' in user_message:
