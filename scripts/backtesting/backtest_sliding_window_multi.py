...
def run_sliding_window_for_tf(df, window_days, tf_name):
    # Determine rows per day based on TF
    ...
    if window_rows < 10: window_rows = 10 # Minimum constraint
    ...
    while test_start_idx < len(df):
        ...
        # Skip if too few data points for optimization
        if len(train_df) < 5:
            test_start_idx += window_rows
            continue
    ...
    return current_equity, max_dd_global, total_trades
...
def main():
    datasets = [
        ...
        ('Monthly', 'data/bybit_btc_usdt_linear_M_full.csv'),
    ]
    ...
    for tf_name, path in datasets:
        ...
        # Test on data from 2023 onwards (Modern era)
        # For Monthly/Weekly data, 2023 might be too short, but let's try.
        start_date = '2023-01-01'
        df = df[df.index >= start_date]
        
        if len(df) < 50:
            print(f"  Skipping {tf_name} (Not enough data since 2023)")
            continue
        ...
        for w_name, w_days in windows:
            # Skip if window is too short for timeframe (e.g. 3 days window for Monthly data makes no sense)
            if tf_name in ['Daily', 'Weekly', 'Monthly'] and w_days < 30:
                continue
            if tf_name in ['Weekly', 'Monthly'] and w_days < 90:
                continue
                
            final, dd, trades = run_sliding_window_for_tf(df, w_days, tf_name)
    ...
    print("\n" + "="*60)
    print(f"{ 'TF':<8} | { 'Window':<10} | { 'Return':<8} | { 'MaxDD':<10}")
    print("-" * 60)
    for r in results:
        print(f"{r['TF']:<8} | {r['Window']:<10} | {r['Return']:>7.2f}% | {r['MaxDD']:>9,.0f}")
    print("="*60)
