...
# ==========================================
# 1. 4H Swing Logic (Adapted from Sliding Window)
# ==========================================
...
# ==========================================
# 2. Sliding Window Optimization for 4H
# ==========================================
...
def run_sliding_window(df, window_days):
    rows_per_day = 6 # 4H data (24h / 4h = 6)
    ...
    print(f"  Running Sliding Window: {window_days} Days (approx {window_rows} bars)...")
    ...
    # 4H Grid Options: Slightly wider than 1H, narrower than Daily
    grid_opts = [1000, 2000, 3000, 5000, 8000]
    ...
    while test_start_idx < len(df):
        ...
        current_equity += period_net # Simple accumulation for simulation
        
        total_profit_accum += prof
        total_fees_accum += fees
        total_trades += buys
        
        # Global DD Tracking (Simplified)
...
def main():
    path = 'data/bybit_btc_usdt_linear_4h_full.csv'
    ...
    # Windows suitable for 4H Swing
    # Longer windows because 4H moves slower
    windows = [
        ('Yearly', 360),
        ('6 Months', 180),
        ('Quarterly', 90),
    ...
    print("\n" + "="*80)
    print(f"4H SWING STRATEGY RESULTS")
