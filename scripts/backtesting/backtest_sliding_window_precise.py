...
def run_sliding_window_true_dd(df, window_days):
    rows_per_day = 288
    # Support float days (e.g. 2.5 days)
...
def main():
    ...
    windows = [2.0, 2.5, 3.0, 3.5, 4.0]
    
    print("\n" + "="*50)
    print(f"{'Window':<10} | {'Return':<10} | {'True MaxDD':<12}")
    print("-" * 50)
    
    for w in windows:
        final, dd = run_sliding_window_true_dd(df, w)
        ret = (final - 1000000) / 1000000 * 100
        print(f"{w:<4} Days     | {ret:>9.2f}% | {dd:>11,.0f}")
