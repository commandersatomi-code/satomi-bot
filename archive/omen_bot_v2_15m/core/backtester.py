...
class DerivativesBacktester:
    ...
    def run(self):
        ...
        for index, row in self.df.iterrows():
            ...
            # Funding rates are applied to the total account equity, not just the position margin
            ...
            # --- 3. Check for Signal ---
            signal = strategy.check_for_signal(row)
            current_price = row['close']

            # --- 4. Execute Trades (Doten Logic) ---
            if signal == "BUY" and position_state != 'LONG':
                ...
                if position_state == 'SHORT':
                    ...
                    logging.info(f"CLOSE SHORT at {current_price:,.2f}. PnL: {pnl:,.2f}. New Margin: {margin_balance:,.2f}")
                    trade_log.append({'date': index, 'type': 'CLOSE_SHORT', 'price': current_price, 'profit_usdt': pnl})
                ...
                logging.info(f"GO LONG at {entry_price:,.2f}. Size: {position_size_btc:.4f} BTC. Liq. Price: {liquidation_price:,.2f}. Fee: {fee:,.2f}")
                ...
                if position_state == 'LONG':
                    ...
                    margin_balance += pnl # Realize PnL
                    logging.info(f"CLOSE LONG at {current_price:,.2f}. PnL: {pnl:,.2f}. New Margin: {margin_balance:,.2f}")
                    trade_log.append({'date': index, 'type': 'CLOSE_LONG', 'price': current_price, 'profit_usdt': pnl})
                ...
                logging.info(f"GO SHORT at {entry_price:,.2f}. Size: {position_size_btc:.4f} BTC. Liq. Price: {liquidation_price:,.2f}. Fee: {fee:,.2f}")
