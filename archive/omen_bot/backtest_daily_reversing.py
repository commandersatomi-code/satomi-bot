    ...
    for index, candle in df.iterrows():
        ...
        if signal == "BUY":
            if position == 'SHORT':
                ...
                usdt_capital += profit
                logging.info(f"{index.strftime('%Y-%m-%d')}: CLOSE SHORT at {candle['close']:,.2f}. Profit: {profit:,.2f} USDT. New capital: {usdt_capital:,.2f} USDT.")
                trade_log.append({
            ...
            if position is None:
                ...
                position = 'LONG'
                logging.info(f"{index.strftime('%Y-%m-%d')}: OPEN LONG ({quality}) at {entry_price:,.2f}. Bought {btc_holdings:.6f} BTC.")
                trade_log.append({
        ...
        # --- Handle SELL signal (go short) ---
            ...
            if position == 'LONG':
                ...
                logging.info(f"{index.strftime('%Y-%m-%d')}: CLOSE LONG at {candle['close']:,.2f}. Profit: {profit:,.2f} USDT. New capital: {usdt_capital:,.2f} USDT.")
                trade_log.append({
            ...
            if position is None:
                ...
                entry_price = candle['close']
                position = 'SHORT'
                logging.info(f"{index.strftime('%Y-%m-%d')}: OPEN SHORT ({quality}) at {entry_price:,.2f}. Sold {btc_holdings:.6f} BTC.")
                trade_log.append({
...
if __name__ == "__main__":
    data_file_path = "/Users/mucbook/バシャール/omen_bot/data/bybit_btc_usdt_linear_daily_full.csv"
    ...
    # trade_log_df.to_csv("/Users/mucbook/バシャール/omen_bot/data/backtest_trade_log.csv")
    # logging.info("Trade log saved to /Users/mucbook/バシャール/omen_bot/data/backtest_trade_log.csv")