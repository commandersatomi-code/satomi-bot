...
def run_backtest(df, initial_jpy_capital, jpy_usdt_rate, trading_fee_rate, trade_capital_percentage):
    logging.info(f"Starting backtest with initial JPY capital: {initial_jpy_capital:,} JPY")
    logging.info(f"Position Sizing: {trade_capital_percentage * 100}% of available capital per trade.")
    ...
    initial_usdt_capital = current_usdt_capital
    ...
    for index, candle in df.iterrows():
        ...
        if signal == "BUY" and not in_position:
            usdt_to_invest = current_usdt_capital * trade_capital_percentage
            if usdt_to_invest > 1: # Minimum trade amount
                btc_to_buy = (usdt_to_invest * (1 - trading_fee_rate)) / candle['close']
                
                btc_holdings = btc_to_buy
                total_investment_usdt = usdt_to_invest
                current_usdt_capital -= usdt_to_invest
                
                entry_price = candle['close']
                in_position = True
                
                total_equity = current_usdt_capital + (btc_holdings * candle['close'])
                trade_log.append({
                    'date': index,
                    'type': 'BUY',
                    'price': entry_price,
                ...
                logging.info(f"{index.strftime('%Y-%m-%d')}: BUY signal ({quality}) at {entry_price:,.2f}. Invested {usdt_to_invest:,.2f} USDT to buy {btc_holdings:.6f} BTC.")
            else:
                logging.info(f"{index.strftime('%Y-%m-%d')}: BUY signal ({quality}) but not enough capital to invest.")

        # --- Handle SELL signal ---
        elif signal == "SELL" and in_position:
            usdt_received = btc_holdings * candle['close'] * (1 - trading_fee_rate)
            profit_usdt = usdt_received - total_investment_usdt
            ...
            total_equity = current_usdt_capital # After selling, all equity is in USDT
            trade_log.append({
                'date': index,
                'type': 'SELL',
                'price': candle['close'],
    ...
    total_profit_jpy = final_jpy_value - (initial_usdt_capital * jpy_usdt_rate)
...
if __name__ == "__main__":
    ...
    TRADE_CAPITAL_PERCENTAGE = 0.20 # Use 20% of available capital for each trade

    final_value, trades = run_backtest(df.copy(), INITIAL_JPY_CAPITAL, JPY_USDT_RATE, TRADING_FEE_RATE, TRADE_CAPITAL_PERCENTAGE)

    # Save trade log to a new file
    trade_log_df = pd.DataFrame(trades)
    new_log_path = "/Users/mucbook/バシャール/omen_bot/data/backtest_trade_log_1h_sized.csv"
    trade_log_df.to_csv(new_log_path)
    logging.info(f"Trade log saved to {new_log_path}")