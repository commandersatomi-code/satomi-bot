                ...
                for _, current_candle in trade_df.iterrows():
                    ...
                    if direction == 'buy':
                        ...
                        if (current_price - entry_price) / entry_price >= tp:
                            ...
                            break
                        # Check SL
                        if (current_price - entry_price) / entry_price <= sl:
                            trade_outcome = sl
                            break
                    elif direction == 'sell':
                        # Check TP (for short position, price goes down)
                        if (entry_price - current_price) / entry_price >= tp:
                            trade_outcome = tp
                            break
                        # Check SL (for short position, price goes up)
                        if (entry_price - current_price) / entry_price <= sl:
                            trade_outcome = sl
                            break
                else:
