...
def validate_new_hypothesis(end_date=None, move_threshold_pct=0.7/100):
    # --- Analysis Parameters ---
    MOVE_WINDOW_HOURS = 6
    # MOVE_THRESHOLD_PCT = 0.7 / 100 # 引数から受け取る
...
def run_threshold_analysis(end_date=None, thresholds=None):
    if thresholds is None:
        thresholds = [0.7, 1.0, 1.5, 2.0, 3.0] # デフォルトの閾値リスト
    
    all_results = []
    for threshold_pct in thresholds:
        result = validate_new_hypothesis(end_date=end_date, move_threshold_pct=threshold_pct/100)
        if result:
