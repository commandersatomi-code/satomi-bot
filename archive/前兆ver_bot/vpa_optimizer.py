...
# ==============================================================================
# 1. DATA FETCHING & INDICATOR CALCULATION (from vpa_analyzer.py)
# ==============================================================================

def fetch_all_data(session, symbol, interval, start_timestamp):
    # (Code from previous script - unchanged)
    all_data = []
...
# ==============================================================================
# 2. OPTIMIZATION & EXPECTED VALUE CALCULATION
# ==============================================================================

def run_analysis_for_params(df, vol_multiplier, body_atr_ratio, look_forward_period=5):
    """Runs the core VPA analysis for a given set of parameters and returns key metrics."""
    ...
    for idx in omen_indices:
        ...
        # We are shorting, so a drop is a win. We measure the max drop.
        max_drawdown_pct = (future_candles['low'].min() - omen_candle['close']) / omen_candle['close'] * 100
        trade_results_pct.append(max_drawdown_pct)
    ...
    # A "win" is a drawdown of more than 1% (i.e., result is less than -1.0)
    ...
    # For EV, we treat wins as positive returns and losses as negative returns
    avg_win_pct = abs(wins.mean()) if not wins.empty else 0
    avg_loss_pct = abs(losses.mean()) if not losses.empty else 0 # Loss is a smaller drop, but still a cost
...
# ==============================================================================
# 3. EXECUTION: GRID SEARCH & VERIFICATION
# ==============================================================================
    ...
    if full_data.empty:
        ...
        for vol_mult in vol_multipliers:
            for body_ratio in body_ratios:
                ...
                stats = run_analysis_for_params(train_data, vol_mult, body_ratio)
                if stats["signals"] > 0:
        ...
        if not optimization_results:
            ...
            print("\n--- Optimization Results Summary (Top 10 by Expected Value) ---")
            print(results_df.head(10).to_string(index=False))
            results_df.to_csv("vpa_optimization_summary.csv", index=False)
            print("\nFull summary saved to vpa_optimization_summary.csv")

            # --- Verification on Test Data ---
            best_params = results_df.iloc[0]
            print(f"\n--- Verifying Best Parameters on Unseen Test Data ---")
            print(f"Best Parameters Found (from Training): Vol Mult: {best_params['vol_mult']:.1f}, Body Ratio: {best_params['body_ratio']:.1f}")
            
            test_stats = run_analysis_for_params(test_data, best_params['vol_mult'], best_params['body_ratio'])
            ...
            if test_stats["ev_pct"] > 0:
                print("\nCONCLUSION: The strategy shows positive expected value on unseen data.")
                print("This suggests the identified edge is ROBUST and not overfitted.")
            else:
                print("\nCONCLUSION: The strategy DID NOT show positive expected value on unseen data.")
                print("This suggests the edge found in training was likely due to overfitting.")
